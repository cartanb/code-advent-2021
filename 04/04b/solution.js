...
const giantSquid = (numArray, boardArray) => {
  const memo = {};
  const winnerArray = [];
  const filteredBoards = [...boardArray];
  for (let i = 0; i < numArray.length; i += 1) {
    const lastDrawn = numArray[i];
    if (!memo[lastDrawn]) {
      memo[lastDrawn] = true;
    }
    for (let j = 0; j < filteredBoards.length; j += 1) {
      const thisBoard = filteredBoards[j];
      if (bingoCheck(thisBoard, lastDrawn)) {
        winnerArray.push([thisBoard, lastDrawn]);
        filteredBoards.splice(j, 1);
        j -= 1;
      }
    }
    if (!filteredBoards.length) {
      return (
        unmarkedSum(winnerArray[winnerArray.length - 1][0]) * lastDrawn
      );
    }
  }
