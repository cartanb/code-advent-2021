...
const ventCheck = (lineArr) => {
  ...
  for (let i = 0; i < lineArr.length; i += 1) {
    ...
    } else if (Math.abs(startCoord[0] - endCoord[0]) === Math.abs(startCoord[1] - endCoord[1])) {
      if (startCoord[0] < endCoord[0]) {
        if (startCoord[1] < endCoord[1]) {
          for (let j = 0; j <= Math.abs(startCoord[0] - endCoord[0]); j += 1) {
            const coord = `${startCoord[0] + j},${startCoord[1] + j}`;
            if (!memo[coord]) {
              memo[coord] = 0;
            }
            memo[coord] += 1;
